...
<head>
  ...
  <link rel="icon" href="http://localhost:9001/dream/images/favicon.png"> 
               ...
               <form name ="qna_content_form" action="qna_delete_proc.jsp" method="post" onsubmit="return confirm('문의글을 정말로 삭제하시겠습니까?');">
                  <input type="hidden" name="qna_id" value="<%= vo.getQna_id() %>" >
                     <table class="review_content" >
                        <tr>
                            <td colspan="4">
                                <a href="http://localhost:9001/dream/qna/qna_list.jsp">
						...
						<tr>
	                        <% if(vo.getMem_id1().equals(sid) || "admin".equals(sid)){ %>
	                            <td colspan="4">
	                                <a href="http://localhost:9001/dream/qna/qna_update.jsp?qna_id=<%= vo.getQna_id()%>">
	                   ...
	                   <table id="qna_comment_list" style="border-bottom: none; width: 1000px; margin:auto" >
		                   ...
		                   <tbody>   
		                      ...
		                      <tr style="border-bottom: 1px dotted grey !important;">
		                          ...
		                          <td>  
	                                   <a href="http://localhost:9001/dream/qna/comm_edit.jsp?qna_id=<%= list.get(i).getQna_id()%>&comm_id=<%=list.get(i).getComm_id() %>">
	                                   <button type="button" class="btn_style1">수정</button>
	                                   </a>
	                                   <a href="http://localhost:9001/dream/qna/comm_delete_proc.jsp?qna_id=<%= list.get(i).getQna_id()%>&comm_id=<%=list.get(i).getComm_id() %>">
